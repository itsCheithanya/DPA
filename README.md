...
```sh
make docker-build docker-push IMG=<some-registry>/dpa:tag
```
...
```sh
make deploy IMG=<some-registry>/dpa:tag
```
...
```sh
make build-installer IMG=<some-registry>/dpa:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/dpa/<tag or branch>/dist/install.yaml
```
...
[Project Paper](https://docs.google.com/document/d/1UH1I7yhQw0EIcwx_1lO8MdwPL_EPDfHHFVn9sSeVSmk/edit?usp=sharing)
## License
